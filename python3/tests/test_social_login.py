...
from lib.social.social_login import login, logout
...
def test_login_step1_login_dialog():
    ...
    response = login(request, login_provider, user_store)
...
def test_login_step2_verify_cridentials_success_and_user_exists():
    ...
    login_provider.verify_cridentials.side_effect = lambda request: ProviderToken('provider_name', request.params.get('user_lookup_token') + '+verified_by_external_provider')
    ...
    user_store.get_user_from_token.assert_called_with(ProviderToken('provider_name', 'user_lookup_token+verified_by_external_provider'))
